...
const buildNormalChangeFromLine = (
  line: string,
  newLineNumber: number,
  oldLineNumber: number,
) => {
...
const expandHunkObjectUp = (
  ...
  if (newStart + nbLines >= 0) {
    const _oldStart =
      prevHunk.oldStart - nbLines - 1 >= 0
        ? prevHunk.oldStart - nbLines - 1
        : 0;
...
const expandHunkObjectDown = (
  ...
  if (prevHunk.oldStart + prevHunk.oldLines < fileLines.length) {
    ...
    const newHunkObject: HunkObject = JSON.parse(
      JSON.stringify({
        ...prevHunk,
        newLines: prevHunk.newLines + _nbLines,
        oldLines: prevHunk.oldLines + _nbLines,
...
export const ExpandableHunk = (props: ExpandableHunkProps) => {
  const lastClass = props.isLast
    ? "hunk-expand last-hunk-expand"
    : "hunk-expand";
  ...
  if (
    hunkDownEnabled &&
    props.hunk.oldStart + props.hunk.oldLines >= props.fileLines.length
  ) {
