...
export const HiddenLargeHunk = (props: HiddenLargeHunkProps & HTMLProps<void>) => {
    const setExpanded = () => {
        const newHunk = {
            ...(props.hunk),
            expanded: true
    ...
    if (props.hunk.changes.length < HUNK_SIZE_LIMIT || props.hunk.expanded)
        ...
        return <Decoration key={props.hunk.content}>
            <div className={`hunk-expand-too-large ${roundedClass}`} onClick={setExpanded}>
                <Col className="hunk-expand-too-large-icon-container" span={8} offset={8}>
