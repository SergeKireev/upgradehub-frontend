...
export const getKey = (u: Upgrade) => {
  return `${u.proxy_address.toLowerCase()}${u.current_impl.toLowerCase()}${
    u.tx_hash
...
export const eventOrderableComparison = (
  a: EventOrderable,
  b: EventOrderable,
) => {
  ...
  if (ts_comparison != 0) {
    ...
    if (tx_index_comparison != 0) {
      ...
      const log_index_comparison =
        compareNumbers(a.log_index, b.log_index) || 0;
...
export function fillPreviousImpl(upgrades: Upgrade[]) {
  upgrades.forEach((u, i) => {
    if (
      i < upgrades.length - 1 &&
      u.current_impl === u.previous_impl &&
      !!upgrades[i + 1]
    ) {
...
export function fillVerifiedOne(
  upgrade: Upgrade,
  verified_index: { [address: string]: boolean },
) {
  if (upgrade.diff) {
    return;
  }
  ...
  if (upgrade.tx_hash === INCEPTION_TX_HASH) {
    ...
    !verified_index[upgrade.previous_impl] &&
    !verified_index[upgrade.current_impl]
  ) {
...
export function fillVerified(
  upgrades: Upgrade[],
  verified_statuses?: VerifiedStatus[],
) {
...
export function trimFirstUpgradeIfEmpty(upgrades: Upgrade[]) {
  ...
  return upgrades.filter(
    (u, i) =>
      !(
        u.unavailable_reason === "PREVIOUS_EQUALS_TARGET" &&
        i === upgrades.length - 1
