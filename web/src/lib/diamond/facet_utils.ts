...
export function compressFacetCutActions(
  facetCutActions: FacetCutSelectorAction[],
...
function eventOrderableComparisonWithAction(
  a: FacetComparable,
  b: FacetComparable,
) {
...
export function createSelectorBuckets(diamondEvents: DiamondEvent[]) {
  ...
  diamondEvents.forEach((d) => {
    ...
    if (!selectorToName[d.selector])
      selectorToName[d.selector] =
        d.function_sig !== null ? d.function_sig : d.selector;
