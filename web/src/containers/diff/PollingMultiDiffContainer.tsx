...
import {
  ...
  PollingComponent,
  PollingStatus,
} from "../polling/PollingComponent";
import {
  ManagedMultiDiffContainer,
  MultiDiffContainer,
} from "./MultiDiffContainer";
...
async function pollForRequestStatus(
  ...
  if (response?.status === "ok" && response?.data?.requested) {
    //Found and already requested, we can continue polling
    return {
      nextAction: "retry",
    };
  } else if (response?.status === "ok" && response?.data?.requestable) {
...
export function PollingMultiDiffContainer(
  props: PollingMultiDiffContainerProps,
) {
