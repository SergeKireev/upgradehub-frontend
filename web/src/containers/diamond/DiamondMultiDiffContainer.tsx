...
function facetActionToUpgrade(
  ...
  if (action.action === 1 && previousAction) {
    ...
    const simpleDiff =
      simpleDiffsIndex[`${previousAddress}_${currentAddress}`]?.diff;
    const upgrade = {
      ...baseUpgrade,
    ...
    else upgrade.unavailable_reason = "INITIALIZATION_UNVERIFIED";
    return upgrade;
...
function upgradesForSelectorsBucket(
  ...
  const simpleDiffsIndex: { [key: string]: SimpleDiff } =
    diamondData.simpleDiffs.reduce((acc, diff) => {
      acc[
        `${diff.previous_impl.toLowerCase()}_${diff.current_impl.toLowerCase()}`
      ] = diff;
      return acc;
    }, {});

  const verifiedStatusesIndex = diamondData.verifiedStatuses.reduce(
    (acc, x) => {
  ...
  const upgrades = bucket.actions
    .map((action, i) => {
      return facetActionToUpgrade(
        ...
        action,
        i > 0 ? bucket.actions[i - 1] : undefined,
      );
    })
...
export const DiamondMultiDiffContainer = (
  props: DiamondMultiDiffContainerProps,
) => {
  const { address, network, bucket, diamondData } = props;

  const upgrades = upgradesForSelectorsBucket(
    address,
    network,
    bucket,
    diamondData,
